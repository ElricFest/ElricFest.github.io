    ...
    function validaUser() {
        ...
        expApellidoP = /[A-Za-zÁÉÍÓÚñáéíóúÑ\']+$/;
        expApellidoM = /[A-Za-zÁÉÍÓÚñáéíóúÑ\']+$/;
        expDomicilio = /^.*(?=.*[0-9])(?=.*[a-zA-ZñÑ\s]).*$/;
        ...
        if (!expApellidoP.test(apellido_paterno)) {
            alert("El Apellido Paterno es unico");
            return false;
        }
        if (!expApellidoM.test(apellido_materno)) {
            alert("El apellido maternpo es unico");
            return false;
            }
        if (!expDomicilio.test(domicilio)){
            alert("El domicilio no cumple con el formato -Morelos #108-");
            return false;
            }
